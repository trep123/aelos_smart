...
```bash
# ROS + leju 动作库已预装，需额外安装：
pip install ultralytics==8.3.0 opencv-python-headless==4.10.0.84 numpy==1.26.4
```

> `opencv-python-headless` 避免与 ROS 自带的 cv2 GUI 库冲突。
...
YOLO 检测类别：`green_square`（绿色方块）、`orange_square`（橙色方块）、`artag`（AR标签）。
...
# 调试模式：直接从返程开始（跳过抓箱子）
python main.py --mode return
```

## 初始模式

通过配置文件或命令行指定机器人启动状态，方便分段调试：

| 模式 | 说明 | ID | step | level |
|------|------|-----|------|-------|
| `full` | 完整流程（默认） | 0 | 1 | start_box |
| `return` | 从返程开始 | 5 | 2 | reverse_moving |
...
# 配置文件持久设置
# config.json → system.initial_mode
```
...
`find_box()` 使用 YOLO 推理替代 HSV 颜色分割，直接检测 `green_square` / `orange_square`。
...
| 节 | 说明 |
|----|------|
| `system` | 工作空间路径、调试开关、初始模式 |
| `yolo` | 模型名、后端、置信度阈值、类别映射 |
| `box_detection` | 方块对准阈值、抓取容忍度、最大接近步数 |
| `box_priority` | 颜色优先级（green > orange） |
| `tag_alignment` | 标签对正默认阈值、9级决策树偏移量 |
| `waypoints` | 全部路径点定义（step1/step2，偏移量+动作序列） |
...
### config_loader.py
配置加载基础设施。`cfg('a','b','c', default=val)` 安全逐级访问，配置缺失自动回退内置默认值。

### actions.py
29个动作包装函数 + `ACTION_REGISTRY` 字典实现字符串→函数调度，`config.json` 中的动作序列可直接引用函数名。

### sensors.py
`RobotSensors` 类聚合摄像头采集（后台线程）和 AR 标签检测，提供线程安全的图像/标签访问。

### box_detector.py
- `find_box()`：YOLO检测方块，按颜色优先级+面积选择目标
- `goto_box()`：先X轴左右对准→再Y轴前后对准→到位后抓取

### tag_navigator.py
- `turn_to_tag()`：9级决策树（L1后退→L2大角度转向→L3快速逼近→L4方向对正→L5粗调平移→L6精调平移→L7慢速逼近→L8精调逼近→L9对正完成）
- 支持左右不对称角度容忍度（`theta_threshold_left` / `theta_threshold_right`）
- `find_waypoint()`：根据tag_id + step查找路径点
...
### yolo_detector.py
YOLO推理封装，支持 ultralytics (PyTorch) 和 ONNX Runtime。提供 `detect()`、`detect_boxes()`、`detect_artag()`。

### main.py
`RobotState` 数据类统一管理运行时状态（消除全局变量），主状态机循环组装各模块。
...
```bash
# 复制到机器人
scp -r demo/ leju@<robot_ip>:/mnt/leju_data/botec/
